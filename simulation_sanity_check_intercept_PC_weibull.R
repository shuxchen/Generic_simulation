...
model_PIV_second <- flexsurvreg(Surv(gaptime, entry2) ~ AG + guidance_before + indexyear, data = genericPIV_postGDUFA_second, dist = "weibullPH")

model_PIV_third <- flexsurvreg(Surv(gaptime, entry2) ~ AG + guidance_before + indexyear, data = genericPIV_postGDUFA_third, dist = "weibullPH")

plot(model_PIV_second, type = "cumhaz")
plot(model_PIV_third, type = "cumhaz")
...
genericPIV_MEPS_2$predicted <- genericPIV_MEPS_2$AG*model_PIV_second$res[3,"est"] + genericPIV_MEPS_2$guidance_before*model_PIV_second$res[4,"est"] + genericPIV_MEPS_2$indexyear*model_PIV_second$res[5,"est"]
genericPIV_MEPS_2$runif <- runif(nrow(genericPIV_MEPS_2), min=0, max=1)
genericPIV_MEPS_2$predicted_t <- (1/(model_PIV_second$res[2,"est"])) * (-log(genericPIV_MEPS_2$runif) / exp(genericPIV_MEPS_2$predicted + log(hazard_sim)))^(1/model_PIV_second$res[1,"est"])  
genericPIV_MEPS_2$predicted_h <- (-log(genericPIV_MEPS_2$runif) / exp(genericPIV_MEPS_2$predicted + log(hazard_sim))) 

genericPIV_MEPS_3$predicted <- genericPIV_MEPS_3$AG*model_PIV_third$res[3,"est"] + genericPIV_MEPS_3$guidance_before*model_PIV_third$res[4,"est"] + genericPIV_MEPS_3$indexyear*model_PIV_third$res[5,"est"]
genericPIV_MEPS_3$runif <- runif(nrow(genericPIV_MEPS_3), min=0, max=1)
genericPIV_MEPS_3$predicted_t <- (1/(model_PIV_third$res[2,"est"])) * (-log(genericPIV_MEPS_3$runif) / exp(genericPIV_MEPS_3$predicted + log(hazard_sim)))^(1/model_PIV_third$res[1,"est"])  
